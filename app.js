...
const LIVE_SOURCE_URL = "https://www.loteriasyapuestas.es/es/juegos-disponibles-online";
...
const betSummaryReintegroRow = document.getElementById("bet-summary-reintegro-row");
...
function setupBettingUI() {
  buildBallGrid("primitiva-grid", 49, "primitiva-number", { startAt: 1, groupSize: 10 });
  buildBallGrid("primitiva-reintegro-grid", 10, "primitiva-reintegro", { startAt: 0, compact: true });
  buildBallGrid("euromillones-grid", 50, "euromillones-number", { startAt: 1, groupSize: 10 });
  buildBallGrid("euromillones-stars-grid", 12, "euromillones-star", { startAt: 1, groupSize: 6, isStar: true });

  document.getElementById("primitiva-random").addEventListener("click", () => {
    betState.primitiva.numbers = pickRandomUnique(6, 1, 49).sort((a, b) => a - b);
    refreshBetButtons();
    renderBetSummary();
  });

  document.getElementById("primitiva-reintegro-random").addEventListener("click", () => {
    betState.primitiva.reintegro = randomBetween(0, 9);
    refreshBetButtons();
    renderBetSummary();
  });

  document.getElementById("euromillones-random").addEventListener("click", () => {
    betState.euromillones.numbers = pickRandomUnique(5, 1, 50).sort((a, b) => a - b);
    refreshBetButtons();
    renderBetSummary();
  });

  document.getElementById("euromillones-stars-random").addEventListener("click", () => {
    betState.euromillones.stars = pickRandomUnique(2, 1, 12).sort((a, b) => a - b);
    refreshBetButtons();
    renderBetSummary();
  });

  document.getElementById("primitiva-joker").addEventListener("change", (event) => {
  ...
  document.querySelectorAll("[data-primitiva-draw]").forEach((button) => {
    button.addEventListener("click", () => {
      document.querySelectorAll("[data-primitiva-draw]").forEach((item) => item.classList.remove("active"));
  ...
  document.querySelectorAll("[data-euromillones-draw]").forEach((button) => {
    button.addEventListener("click", () => {
      document.querySelectorAll("[data-euromillones-draw]").forEach((item) => item.classList.remove("active"));
      button.classList.add("active");
...
function setActiveBetGame(gameId) {
  ...
  gameTabs.forEach((tab) => tab.classList.toggle("active", tab.dataset.gameTab === gameId));
  betPanels.forEach((panel) => panel.classList.toggle("active", panel.dataset.betPanel === gameId));
  renderBetSummary();
}
...
function renderBetSummary() {
  ...
  if (gameId === "primitiva") {
    ...
    betSummaryReintegro.textContent = betState.primitiva.reintegro === null ? "-" : String(betState.primitiva.reintegro);
    betSummaryExtra.textContent = betState.primitiva.joker ? "Con Joker" : "Sin Joker";
    betSummaryDraw.textContent = betState.primitiva.draw === "semanal" ? "Semanal" : "Próximo sorteo";
  ...
  if (gameId === "euromillones") {
    ...
    betSummaryNumbers.textContent = formatBallList(betState.euromillones.numbers);
    ...
    betSummaryDraw.textContent = betState.euromillones.draw === "semana" ? "Dos sorteos de la semana" : "Próximo sorteo";
    betSummaryTotal.textContent = formatMoney(total);
  }
...
/**
 * Aplica al arrancar el tema guardado previamente en localStorage.
 */
...
/**
 * Actualiza el texto del botón de tema según el modo actual.
 */
